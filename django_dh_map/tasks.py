...
from .settings import MEDIA_ROOT_DIR, MEDIA_URL, \
    DH_MAP_OVERHEAD_TILE_SIZE, DH_MAP_PANORAMA_TILE_SIZE, \
    DH_MAP_FFMPEG, DH_MAP_VIDEO_RESOLUTIONS, \
    DH_MAP_VIDEO_SEGMENT_DURATION, DH_MAP_VIDEO_THUMBNAIL_INTERVAL
...
def task_overhead_map_tiles_generator(object_pk):
    ...
    if map.has_image():
        ...
        map.tile_size = min(DH_MAP_OVERHEAD_TILE_SIZE, map.width, map.height)
        map.min_zoom = 1
        map.max_zoom = int(math.ceil(math.log(float(max(map.width, map.height)) / map.tile_size, 2)))
...
def task_panorama_map_tiles_generator(object_pk):
    ...
    if map.has_image():
        ...
        cube_dict = e2c(
            np.array(Image.open(image_path.absolute())),
            face_w=(map.width // 4),
            mode='bilinear',
            cube_format='dict'
        )
        ...
        map.cube_size = 8 * int((360 / HAOV) * map.width / math.pi / 8)
        map.tile_size = min(DH_MAP_PANORAMA_TILE_SIZE, map.cube_size)
        map.min_zoom = 1
...
def task_video_stream_generator(object_pk):
    ...
    if video.has_original():
        ...
        resolution_maps = []
        resolutions = []
        for index, resolution_dict in enumerate(DH_MAP_VIDEO_RESOLUTIONS):
...
-seg_duration {DH_MAP_VIDEO_SEGMENT_DURATION} -sc_threshold 0 -b_strategy 0 -g 100 -keyint_min 100
                ...
                with open(thumbnails_vtt_path.absolute(), 'w') as thumbnails_vtt_file:
                    ...
                    for index, temp_file in enumerate(temp_files):
                        with Image.open(temp_file.absolute()) as temp_image_file:
                            ...
                            start_time = index * DH_MAP_VIDEO_THUMBNAIL_INTERVAL
                            end_time = (index + 1) * DH_MAP_VIDEO_THUMBNAIL_INTERVAL
