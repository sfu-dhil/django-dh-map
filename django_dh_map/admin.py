...
from .models import Map, OverheadImageMap, PanoramaImageMap, AbstractImageMap, XyzMap, \
    InfoPage, Feature, ContentBlock, ContentBlockRichText, ContentBlockImage, ContentBlockImageGallery, \
...
# IconFont
from .font_icons import FontIcon
...
class DjangoDhMapAdminMixin:
  class Media:
    css = { 'all': ['django_dh_map/admin/dist/admin.css'] }
    js = [JS('django_dh_map/admin/dist/admin.js', {'type': 'module'})]
...
class SortablePolymorphicParentModelAdmin(PolymorphicParentModelAdmin):
    @property
    def change_list_template(self):
        return "admin/change_list.html"

class MapAdminChildAdmin(DjangoDhMapAdminMixin, PolymorphicChildModelAdmin):
    base_form = BaseMapAdminForm

@admin.register(XyzMap)
class XyzMapChildAdmin(MapAdminChildAdmin):
    base_form = XyzMapForm
    base_model = XyzMap

@admin.register(OverheadImageMap)
class OverheadImageMapChildAdmin(MapAdminChildAdmin):
    base_form = OverheadImageMapForm
    base_model = OverheadImageMap

    def get_fields(self, request, obj=None):
        fields = super().get_fields(request, obj)
        if not obj or not obj.has_tiles():
            fields.remove('map_geojson')
            fields.remove('properties')
        return fields

@admin.register(PanoramaImageMap)
class PanoramaImageMapChildAdmin(MapAdminChildAdmin):
    base_form = PanoramaImageMapForm
...
@admin.register(Map)
class MapParentAdmin(DjangoDhMapAdminMixin, SortableAdminMixin, SortablePolymorphicParentModelAdmin):
    child_models = (XyzMap, OverheadImageMap, PanoramaImageMap)
    list_filter = [PolymorphicChildModelFilter]
    polymorphic_list = True
    list_per_page = 20
    ordering = ['position']

    list_display = ('position', 'id', '_type', 'label', '_preview', '_status')
    list_display_links = ('id', '_type', 'label')
    ...
    def _preview(self, obj):
        if isinstance(obj, XyzMap) and obj.url:
            return mark_safe(f'<div class="openlayers-preview-app" data-map-id="{obj.pk}"></div>')
        elif isinstance(obj, OverheadImageMap) and obj.tiles_dir:
            return mark_safe(f'<div class="openlayers-preview-app" data-map-id="{obj.pk}"></div>')
        elif isinstance(obj, PanoramaImageMap) and obj.tiles_dir:
            return mark_safe(f'<div class="pannellum-preview-app" data-map-id="{obj.pk}"></div>')
        return ''
    ...
    def _status(self, obj):
        ...
        if isinstance(obj, AbstractImageMap):
            tiles_status_str = mark_safe(f'{get_success_failure_status_tick(obj.has_tiles())} Map Tiles <br />')
...
class ContentBlockInline(SortableHiddenMixinTinyMceFix, NestedStackedPolymorphicInline):
    model = ContentBlock
    ...
    class ContentBlockImageGalleryInline(SortableHiddenMixinTinyMceFix, NestedStackedPolymorphicInline.Child):
        ...
        fields = ['_gallery_preview_tag', 'position']

        readonly_fields = ['_gallery_preview_tag']
        def _gallery_preview_tag(self, obj):
            images = obj.images.all()
            images_json = json.dumps(ContentBlockImageGalleryImageSerializer(images, many=True).data) if len(images) > 0 else None
            return mark_safe(f'<div class="admin-gallery-preview-app" data-gallery-images-json="{escape(images_json)}"></div>') if images_json else 'N/A'
        _gallery_preview_tag.short_description = 'Preview'
    ...
    class ContentBlockImageBeforeAndAfterInline(SortableHiddenMixinTinyMceFix, NestedStackedPolymorphicInline.Child):
        ...
        fieldsets = (
            (None, { 'fields': [
                ('_before_after_preview_tag', 'caption',), 'position',
            ], }),
...
class AbstractContentItemAdmin(DjangoDhMapAdminMixin, NestedPolymorphicModelAdmin):
...
class InfoPageAdmin(SortableAdminMixin, AbstractContentItemAdmin):
...
class IconInline(NestedStackedPolymorphicInline):
    ...
    # # TODO: FIX: doesn't work well at the moment
    # class IconFontInline(SortableHiddenMixinTinyMceFix, NestedStackedPolymorphicInline.Child):
    #     model = IconFont
    #     fields = ['class_value']
    #     def formfield_for_dbfield(self, db_field, **kwargs):
    #         if db_field.name == 'class_value':
    #             kwargs['widget'] = Select2Widget(attrs={
    #                     'data-placeholder': 'Click to select a Bootstrap or Fontawesome icon',
    #                 },
    #                 choices=FontIcon.choices,
    #             )
    #         return super().formfield_for_dbfield(db_field, **kwargs)

    class IconImageInline(SortableHiddenMixinTinyMceFix, NestedStackedPolymorphicInline.Child):
        model = IconImage
        fields = [('_thumbnail_tag', 'original')]
        readonly_fields = ['_thumbnail_tag']
        def _thumbnail_tag(self, obj):
            return mark_safe(f'<img class="admin-image-preview" style="border-radius: 50%" src="{obj.icon_thumbnail.url}" />') if obj.icon_thumbnail else 'N/A'
        _thumbnail_tag.short_description = 'Preview'

    class IconNumberedInline(SortableHiddenMixinTinyMceFix, NestedStackedPolymorphicInline.Child):
        model = IconNumbered
        fields = ['number']
...
class FeatureAdmin(AbstractContentItemAdmin):
    ...
    def get_fields(self, request, obj=None):
        fields = super().get_fields(request, obj)
        if obj and 'properties' in fields and DH_MAP_CI_FEATURE_PROPERTIES_SCHEMA == DH_MAP_DEFAULT_PROPERTIES_SCHEMA:
            fields.remove('properties')
        return fields